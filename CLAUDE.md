...
| Page | Password? | Mechanism | Status |
|---|---|---|---|
| Block Sites | No | Hard redirect via `background/index.ts` + `webNavigation` — categories + custom list | ✅ Built |
| Reel Blocks | No | Same mechanism as Block Sites, presets scoped to reel-specific paths | Not built |
| Feed Blocks | No | **Content script**, not a redirect — hides DOM elements per site | Not built |
| Password Protection | Yes | **Content script overlay**, not a redirect. No timer — refresh re-prompts | Not built |
| Limiter | No | Hard redirect once a daily time budget runs out | Not built |
| Focus Mode | — | Coming Soon placeholder | Placeholder (matches design) |
| Insights | — | Coming Soon placeholder | Placeholder (matches design) |
| Settings | — | Extension-wide preferences | Not built |
| About Us | — | Static brand/info page | Not built |

**Do not conflate the three mechanisms.** Hard-redirect features only need `background/index.ts` +
`webNavigation` — no content script. Feed Blocks and Password Protection both need content scripts,
for different reasons (element-hiding vs. an overlay gate), and neither should be built as a
redirect.
...
- `FocusLock-Page-List-v2.md` / `v3.md` — the reasoning behind the Password Protection redesign and
  Feed Blocks addition, if more context is ever needed on *why*.
